...
def plot_all_points_model_comparisons(points_of_interest, models_data_mapping, 
                                     buffer_days=15, time_slice=None, save_dir=None):
    """
    Plot all points with model comparisons in subplots
    ...
    print("PLOTTING ALL POINTS WITH MODEL COMPARISONS")
    print("=" * 60)
    ...
    for point in points_of_interest:
        print(f"\n{'='*50}")
        print(f"POINT: {point['name']} (lat={point['lat']}, lon={point['lon']})")
        print(f"{'='*50}")
        ...
        if save_dir:
            safe_name = point['name'].replace('/', '_').replace('\\', '_')
            filename = os.path.join(save_dir, f"{safe_name}_model_comparison.png")
            fig.savefig(filename, dpi=150, bbox_inches='tight')
            print(f"  Saved to {filename}")
        
        plt.show()
        
        # Create summary for this point
        point_summary = {
            'point': point['name'],
            'lat': point['lat'],
            'lon': point['lon']
        ...
        # Add event counts for each model
        models = list(models_data_mapping.keys())
        for idx, (model_name, events_data) in enumerate(zip(models, events_data_list)):
            if events_data is not None:
                event_count = int(events_data.event_count.values)
                point_summary[f'{model_name}_events'] = event_count
                ...
                point_summary[f'{model_name}_events'] = 'Error'
                point_summary[f'{model_name}_avg_duration'] = 'Error'
                point_summary[f'{model_name}_max_duration'] = 'Error'
        
        all_summaries.append(point_summary)
    
    # Print summary table
    print_summary_table(all_summaries, models_data_mapping)
